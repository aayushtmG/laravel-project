...
<div class="md:container md:mx-auto px-6 md:px-20  my-8 flex flex-col">
  ...
  <div class="flex justify-between items-center">
    ...
    <a href="{{route('admin.notices.show')}}" class="btn hover:bg-slate-800 hover:text-white">All Notcies</a>
  </div>
  <form action="{{route('notices.edit')}}" class=" flex max-md:flex-col-reverse gap-10 py-4" method="POST" enctype="multipart/form-data">
    @csrf
   <div class="flex flex-col gap-4 shadow-md p-4 rounded-md w-full ">
    ...
    notice Information:</h1>
      ...
      <input type="text" id="notice_title" name="title" class="border border-slate-300 rounded-sm p-2" placeholder="Enter Notice Title" value='{{$notice->title}}'>
      @if($errors->has('title'))
      ...
      <div class="flex flex-col">
        ...
        <textarea id="notice_description" name="description" class="border border-slate-300 rounded-sm min-h-[200px] p-3"
    ...
    <div class="mb-3 text-center h-[300px] max-w-[300px] border mx-auto">
        <img id="image_preview" src="{{ asset('images/notices/'.$notice->image) }}" alt="No Image Available" class="w-[300px] h-[300px] object-cover" >
    </div>
