...
<div class="md:container md:mx-auto px-6 md:px-20  my-8 flex flex-col">
  ...
  <div class="flex justify-between items-center">
    ...
    <a href="{{route('admin.members.show')}}" class="btn hover:bg-slate-800 hover:text-white">All Member</a>
  </div>
  <form action="{{route('members.create')}}" class=" flex max-md:flex-col-reverse gap-10 py-4" method="POST" enctype="multipart/form-data">
    @csrf
   <div class="flex flex-col gap-4 shadow-md p-4 rounded-md w-full ">
    ...
    Member Information:</h1>
      ...
      <input type="text" id="member_name" name="name" class="border border-slate-300 rounded-sm p-1" placeholder="Enter Name" value="{{old('name')}}"/>
      @if($errors->has('name'))
      ...
      <div class="flex gap-4 max-md:flex-col">
        ...
        <input type="email" id="email" name="email" class="border border-slate-300 rounded-sm p-1 " placeholder="Enter Email" value="{{old('email')}}"/> 
      @if($errors->has('email'))
      ...
      <div class="flex gap-4 max-md:flex-col">
        ...
        <input type="text" id="position" name="position" class="border border-slate-300 rounded-sm p-1 " placeholder="Enter Position" value="{{old('position')}}"/> 
